...
class StudentAddImpl implements StudentAdd {            // 学生添加实现类
    ...
    @Override
    public Student addStudent(int no, String name, int professiong, double score){
        if (no <= 0 || score < 0 || score > 100 || (profession != 1 && profession != 2 && profession != 3)) {
            throw new IllegalArgumentException("输入参数不合法");
        }
        return new Student(no, name, professiong, score);
    }
}
...
public class StudentTest {
    public static void main(String[] args) {
        ...
        StudentSelect studentSelect = new StudentSelectImpl();        // 查询学生信息
